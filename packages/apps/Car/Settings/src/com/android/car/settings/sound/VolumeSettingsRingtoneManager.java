...
public class VolumeSettingsRingtoneManager {
    ...
    private Ringtone lazyLoadRingtone(int group, int usage) {
        ...
        /* 当Ringtone.stop()执行之后，会导致Ringtone中的mLocalPlayer停止并设置为空，再次调用play()
         * 时会使用mRemotePlayer进行播放。
         * mRemotePlayer为SystemUI.RingtonePlayer中创建的RingtonePlayer对象，导致AudioFlinger获取Uid异常。
         */
        Ringtone ringtone = RingtoneManager.getRingtone(mContext, getRingtoneUri(usage));
        if (ringtone == null) {
            return null;
