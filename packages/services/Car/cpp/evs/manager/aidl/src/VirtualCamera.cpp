...
ScopedAStatus VirtualCamera::startVideoStream(const std::shared_ptr<IEvsCameraStream>& receiver) {
    ...
    mCaptureThread = std::thread([this]() {
        ...
        while (true) {
            ...
            if (!mFramesReadySignal.wait_for(lock, kFrameTimeout, [this]() REQUIRES(mFrameDeliveryMutex) {
                    // Stops waiting if
                ...
                std::lock_guard<std::recursive_mutex> lock(mFramesHeldMutex);
                if (mFramesHeld.size() > 0 && mStream != nullptr) {
...
ScopedAStatus VirtualCamera::stopVideoStream() {
    if (mStreamState == RUNNING) {
        ...
        if (mCaptureThread.get_id() == std::this_thread::get_id()) {
