...
#if RKAVM
    if(RenderAvmRk::mProcess)
    {
        delete RenderAvmRk::mProcess;
        RenderAvmRk::mProcess = nullptr;
    }
#endif

    RenderBase::terminate();
...
bool EvsStateControl::configureEvsPipeline(State desiredState) {
...
#if RKAVM
        //printf("get vehicle state is %d curRenderType %d\n", desiredState, mCurRenderType);
        if(mCurRenderType != RenderType::AVM){
            mDesiredRenderer = std::make_unique<RenderAvmRk>(mEvs,
                                                            mCameraList[PARKING], //WE NEED ALL CAMERA
                                                            mConfig);
            if (!mDesiredRenderer) {
    ...
    if (mCurRenderType == RenderType::NONE && mCurrentRenderer) {
        mCurrentRenderer->deactivate();
    ...
    if (mCurRenderType == RenderType::NONE && mCurrentRenderer == nullptr) {
...
#if RKAVM
        //Note: For RKAVM, we init only once,if state changed just send cmd
        ...
        enum RK_AVM_CMD rk_avm_cmd;

        switch(desiredState)
        {
        case REVERSE:
            rk_avm_cmd = rk_cmd_render_back_top_fish_eye;//rk_cmd_render_back_3D also
            break;
        case PARKING:
            rk_avm_cmd = rk_cmd_render_3d_view_adjuest;
            break;
        case LEFT:
            rk_avm_cmd = rk_cmd_render_left_top_2D;
            break;
        case RIGHT:
            rk_avm_cmd = rk_cmd_render_right_top_2D;
            break;
        default:
            rk_avm_cmd = rk_cmd_render_3d_view_adjuest;
            break;
        ...
        if(mCurRenderType == RenderType::AVM)
        {
            RenderAvmRk *curRender = (RenderAvmRk*)(mCurrentRenderer.get());
            curRender->AVMSetSurroundType(rk_avm_cmd);
        }
