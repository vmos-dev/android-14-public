...
#if RKAVM
bool VideoTex::refreshGraphicBuffer() {
    if (!mStreamHandler->newFrameAvailable()) {
        // No new image has been delivered, so there's nothing to do here
        return false;
    }
    ...
    myBuffer = new GraphicBuffer(nativeHandle,
                                    GraphicBuffer::CLONE_HANDLE,
                                    pDesc->width,
                                    pDesc->height,
                                    pDesc->format,
                                    1,//pDesc->layers,
                                    GRALLOC_USAGE_HW_TEXTURE, /*need FB*/
                                    pDesc->stride);
    if (!myBuffer) {
        LOG(ERROR) << "Failed to allocate GraphicBuffer to wrap image handle";
        return false;
     ...
     if (auto status = myBuffer->initCheck(); status != android::OK) {
        LOG(ERROR) << "Failed to initialize the graphic buffer, error = "
                   << android::statusToString(status);
        // Same here.
        return true;
    ...
    // Get a GL compatible reference to the graphics buffer we've been given
    EGLint eglImageAttributes[] = {EGL_IMAGE_PRESERVED_KHR, EGL_TRUE, EGL_NONE};
    EGLClientBuffer clientBuf = static_cast<EGLClientBuffer>(myBuffer->getNativeBuffer());
    mKHRimage = eglCreateImageKHR(mDisplay, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID, clientBuf,
                                  eglImageAttributes);
    if (mKHRimage == EGL_NO_IMAGE_KHR) {
        const char* msg = getEGLError();
...
VideoTex* createVideoTexture(const std::shared_ptr<IEvsEnumerator>& pEnum, const char* evsCameraId,
    ...
    if (!streamCfg) {
        ...
        if (auto status = pEnum->openCamera(evsCameraId, emptyConfig, &pCamera); !status.isOk()) {
