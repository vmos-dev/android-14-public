...
public final class CarOccupantZoneService extends ICarOccupantZone.Stub
    ...
    public static final String CAR_SERVICE_INIT_TIMING_TAG = "CarOccupantZoneService.InitTiming";
    ...
    public static final String OCCUPANT_ZONE_CONFIG = "system/car/occupant_zone_config.xml";
    public static final String TAG_OCCUPANT_ZONE_VERSION = "occupant_zone_version";
    public static final String VALUE_TAG_OCCUPANT_ZONE_VERSION = "1";
    public static final String TAG_OCCUPANT = "occupant";
    public static final String TAG_OCCUPANT_ZONE = "occupant_zone";
    public static final String TAG_OCCUPANT_ZONE_ID = "zone_id";
    public static final String TAG_OCCUPANT_USER_ID = "user_id";
    ...
    final ExperimentalCarUserService.PassengerCallback mPassengerCallback =
            new ExperimentalCarUserService.PassengerCallback() {
                ...
                public void onPassengerStarted(@UserIdInt int passengerId, int zoneId) {
                    Slogf.w(TAG, "onPassengerStarted passengerId = " + passengerId + ", zoneId = " + zoneId);
                    if (mActiveOccupantConfigs.get(zoneId) != null && mActiveOccupantConfigs.get(zoneId).displayInfos.size() > 0) {
                        boolean isSuccess = ActivityManagerHelper.startUserInBackgroundVisibleOnDisplay(passengerId, mActiveOccupantConfigs.get(zoneId).displayInfos.get(0).display.getDisplayId());
                        Slogf.w(TAG, "onPassengerStarted isSuccess = " + isSuccess);
                        if (isSuccess) {
        ...
        ZoneUserBindingHelper helper = new ZoneUserBindingHelper() {
            ...
            public boolean assignUserToOccupantZone(@UserIdInt int userId, int zoneId) {
                ...
                synchronized (mLock) {
                    ...
                    zoneConfig.userId = userId;
                    Slogf.w(TAG, "assignUserToOccupantZone true! userId = " + userId + ", zoneId = " + zoneId);
                    return true;
                }
            }
    ...
    public boolean assignProfileUserToOccupantZone(int occupantZoneId, @UserIdInt int userId) {
        ...
        Slogf.d(TAG, "assignProfileUserToOccupantZone zoneid=" + occupantZoneId + " ; userId=" + userId);
    ...
    private void parseOccupantZoneConfigsLocked() {
        ...
        for (String config : res.getStringArray(R.array.config_occupant_zones)) {
            ...
            int userId = getUserWithOccupantZone(zoneId);
            if (userId != CarOccupantZoneManager.INVALID_USER_ID) {
    ...
    private void addDisplayInfoToOccupantZoneLocked(int zoneId, DisplayInfo info) {
        ...
        boolean mEnablePassengerSupport = mContext.getResources().getBoolean(R.bool.enablePassengerSupport);
        ExperimentalCarUserService experimentalCarUserService = CarLocalServices
                .getService(ExperimentalCarUserService.class);
        if (mEnableProfileUserAssignmentForMultiDisplay && mEnablePassengerSupport
                && experimentalCarUserService != null) {
            if (zoneId != mDriverZoneId && occupantConfig.displayInfos.size() > 0) {
                boolean isSuccess = experimentalCarUserService.AddNewPassengersUser(zoneId);
                Slogf.w(TAG, "addDisplayInfoToOccupantZoneLocked AddNewPassengersUser to Zone(" + zoneId + ") isSuccess = " + isSuccess);
            }
        }
    }
    ...
    private boolean handleUserChangesLocked() {
        ...
        for (int i = 0; i < mActiveOccupantConfigs.size(); ++i) {
            ...
            if (!isUserVisible(UserHandle.of(config.userId))) {
                if (DBG) Slogf.w(TAG, "Unassigned no longer visible user:%d", config.userId);
                //config.userId = CarOccupantZoneManager.INVALID_USER_ID;
                config.userId = getUserWithOccupantZone(zoneId);
                changed = true;
            }
        }
    ...
    private void reLauncherRemoveTask(int displayId) {
        if (mDisplayManager.getDisplay(displayId) == null) {
            return;
        }
        if (mEnableProfileUserAssignmentForMultiDisplay && UserManager.isHeadlessSystemUserMode()) {
            ActivityManager activityManager = (ActivityManager) mContext.getSystemService(ActivityManager.class);
            List<ActivityManager.RunningTaskInfo> runningTasks = activityManager.getRunningTasks(Integer.MAX_VALUE);
            for (ActivityManager.RunningTaskInfo task : runningTasks) {
                int userId = TaskInfoHelper.getUserId(task);
                int taskDisplayId = TaskInfoHelper.getDisplayId(task);
                int occupantZoneId = getOccupantZoneIdForUserId(userId);
                int targetDisplayId = getDisplayForOccupant(occupantZoneId, CarOccupantZoneManager.DISPLAY_TYPE_MAIN);
                if (DBG) {
                    Slogf.w(TAG, "task = " + TaskInfoHelper.toString(task));
                    Slogf.w(TAG, "userId = " + userId + ", taskDisplayId = " + taskDisplayId + ", occupantZoneId = " + occupantZoneId + ", targetDisplayId = " + targetDisplayId + ", displayId = " + displayId);
                }
                if (occupantZoneId != OccupantZoneInfo.INVALID_ZONE_ID && targetDisplayId != taskDisplayId && targetDisplayId == displayId) {
                    ActivityManagerHelper.moveRootTaskToDisplay(task.taskId, targetDisplayId);
                }
            }
        }
    }
    ...
    private void handleDisplayChange(int displayId) {
        ...
        reLauncherRemoveTask(displayId);
    }
    ...
    private void setOccupantZoneAndUserId() {
        ...
        t.traceBegin(TAG + ".setOccupantZoneAndUserId");

        File writeOccupantUserSettings = new File(Environment.getDataDirectory(), OCCUPANT_ZONE_CONFIG);
        AtomicFile mAtomicFile = new AtomicFile(writeOccupantUserSettings);
        FileOutputStream fileOutputStream = null;
        try {
            fileOutputStream = mAtomicFile.startWrite();
        } catch (Exception e) {
            Slogf.e(TAG, "setOccupantZoneAndUserId startWrite failed", e);
        }
        ...
        try {
            ...
            serializer.startTag(null, TAG_OCCUPANT);

            serializer.startTag(null, TAG_OCCUPANT_ZONE_VERSION);
            serializer.text(VALUE_TAG_OCCUPANT_ZONE_VERSION);
            serializer.endTag(null, TAG_OCCUPANT_ZONE_VERSION);

            for (int i = 0; i < mActiveOccupantConfigs.size(); i++) {
                int key = mActiveOccupantConfigs.keyAt(i);
                OccupantConfig value = mActiveOccupantConfigs.valueAt(i);
                if (DBG) {
                    Slogf.w(TAG, "key = " + key + ", value = " + value.userId);
                }
                serializer.startTag(null, TAG_OCCUPANT_ZONE);
                serializer.attribute(null, TAG_OCCUPANT_ZONE_ID, String.valueOf(key));
                serializer.startTag(null, TAG_OCCUPANT_USER_ID);
                serializer.text(String.valueOf(value.userId));
                serializer.endTag(null, TAG_OCCUPANT_USER_ID);
                serializer.endTag(null, TAG_OCCUPANT_ZONE);
            }

            serializer.endTag(null, TAG_OCCUPANT);
            serializer.endDocument();
            success = true;
        } catch (Exception e) {
            Slogf.e(TAG, "setOccupantZoneAndUserId write failed", e);
        } finally {
            if (DBG) {
                Slogf.w(TAG, "setOccupantZoneAndUserId write success: " + success);
            }
            if (success) {
    ...
    private int getUserWithOccupantZone(int zoneId) {
        int userId = CarOccupantZoneManager.INVALID_USER_ID;
        LimitedTimingsTraceLog t = new LimitedTimingsTraceLog(
                CAR_SERVICE_INIT_TIMING_TAG, TraceHelper.TRACE_TAG_CAR_SERVICE,
                CAR_SERVICE_INIT_TIMING_MIN_DURATION_MS);
        t.traceBegin(TAG + ".getUserWithOccupantZone");
        FileInputStream fis = null;
        try {
            File readOccupantUserSettings = new File(Environment.getDataDirectory(), OCCUPANT_ZONE_CONFIG);
            AtomicFile mAtomicFile = new AtomicFile(readOccupantUserSettings);
            fis = mAtomicFile.openRead();
            boolean success = false;
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                ...
                switch (eventType) {
                    case XmlPullParser.START_TAG:
                        if (TAG_OCCUPANT_ZONE.equals(parser.getName())) {
                            int fileZoneId = Integer.parseInt(parser.getAttributeValue(null, TAG_OCCUPANT_ZONE_ID));
                            if (fileZoneId == zoneId) {
                                success = true;
                            }
                            if (DBG) {
                                Slogf.w(TAG, "TAG_OCCUPANT_ZONE zoneId = " + fileZoneId);
                            }
                            break labelswitch;
                        }
                        if (success) {
                            if (TAG_OCCUPANT_USER_ID.equals(parser.getName())) {
                                userId = Integer.parseInt(parser.nextText());
                                if (DBG) {
                                    Slogf.w(TAG, "TAG_OCCUPANT_USER_ID userId = " + userId);
                                }
                            }
                            break labelwhile;
                        }
            ...
            Slogf.e(TAG, "getUserWithOccupantZone read failed");
        } finally {
            try {
                if (fis != null) {
                    fis.close();
                ...
                Slogf.e(TAG, "getUserWithOccupantZone close failed");
            }
        }
        t.traceEnd();
        return userId;
