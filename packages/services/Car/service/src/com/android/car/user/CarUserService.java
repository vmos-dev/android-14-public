...
import static android.car.builtin.os.UserManagerHelper.isAdminUser;
...
public final class CarUserService extends ICarUserService.Stub implements CarServiceBase {
        ...
        if (isPlatformVersionAtLeastU()) {
            if (UserManager.isHeadlessSystemUserMode()) {
                CarServiceHelperWrapper.getInstance().runOnConnection(()-> {
                    Slogf.d(TAG, "In Headless, wati CarServiceHelper connect to initBootUser!");
    ...
    @Nullable
    private OccupantZoneInfo getOccupantZoneForUserId(int userId) {
        CarOccupantZoneService zoneService = CarLocalServices.getService(
                CarOccupantZoneService.class);
        OccupantZoneInfo occupantZoneInfo = zoneService.getOccupantZoneForUser(UserHandle.of(userId));
        return occupantZoneInfo != null ? occupantZoneInfo : null;
    }
    ...
    private void onUserStarting(@UserIdInt int userId) {
        ...
        if (userId == ActivityManager.getCurrentUser() && !UserManagerHelper.isFullUser(mUserManager, UserHandle.of(userId))) {
            if (DBG) {
    ...
    private void assignVisibleUserToZone(@UserIdInt int userId) {
        ...
        /* Get the OccupanZone based on the UserId */
        OccupantZoneInfo zoneInfo = getOccupantZoneForUserId(userId);
        if (zoneInfo == null) {
