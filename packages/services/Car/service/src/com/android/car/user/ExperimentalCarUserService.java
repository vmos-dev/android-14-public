...
public final class ExperimentalCarUserService extends IExperimentalCarUserService.Stub
    ...
    public ExperimentalCarUserService(Context context, CarUserService carUserService,
        ...
        mCarOccupantZoneService = CarLocalServices.getService(CarOccupantZoneService.class);
    }
    ...
    public UserHandle createPassenger(String name, @UserIdInt int driverId) {
        ...
        UserHandle user = userManager.createProfile(name, UserManager.USER_TYPE_PROFILE_MANAGED,
                /* disallowedPackages= */ new HashSet<String>());
    ...
    public boolean startPassenger(@UserIdInt int passengerId, int zoneId) {
        ...
        synchronized (mLock) {
            int[] mZoneDisplays = getCarOccupantZoneService().getAllDisplaysForOccupantZone(zoneId);
            if (mZoneDisplays.length > 0) {
                int mZoneDisplayId = mZoneDisplays[0];
                if (checkUserStateAndDisplay(passengerId, mZoneDisplayId)) {
                    Slogf.d(TAG, "In Zone(" + zoneId + "), passengerId: " + passengerId + " has assigned to displayId: " + mZoneDisplayId);
                    return true;
                }
                if (!ActivityManagerHelper.startUserInBackgroundVisibleOnDisplay(passengerId, mZoneDisplayId)) {
                    Slogf.w(TAG, "could not start passenger with displayId(" + mZoneDisplayId + ")");
                }
            } else {
                Slogf.w(TAG, "zoneId(" + zoneId + ") " + " has no display. Don't Start Passenger!");
                return false;
            }
    ...
    private void onUserSwitching(@UserIdInt int fromUserId, @UserIdInt int toUserId) {
        ...
        if (mEnablePassengerSupport && isPassengerDisplayAvailable()) {
            ...
            int count = setupAndStartPassengers(toUserId);
            Slogf.i(TAG, "Total Passengers = " + count);
        }
        t.traceEnd();
    }
    ...
    private CarOccupantZoneService getCarOccupantZoneService() {
        if (mCarOccupantZoneService == null) {
            mCarOccupantZoneService = CarLocalServices.getService(CarOccupantZoneService.class);;
        }
        return mCarOccupantZoneService;
    }

    /* check if the user is unlocked and current display is the same as the one assigned to the user */
    private boolean checkUserStateAndDisplay(int userId, int displayId) {
        boolean ret = mUserManager.isUserUnlocked(UserHandle.of(userId));
        int mCurrentUserId = CarServiceHelperWrapper.getInstance().getUserAssignedToDisplay(
                displayId);
        return ret && (mCurrentUserId == userId);
    }
     ...
     */
    public int setupAndStartPassengers(int driverId) {
        int currentUser = ActivityManager.getCurrentUser();
        ...
        for (int occupantType : occupantTypes) {
            ...
            synchronized (mLock) {
                if (mZoneUserBindingHelper == null) {
                    Slogf.e(TAG, "implementation is not delegated");
                    return 0;
                }
                helper = mZoneUserBindingHelper;
            }
            ...
            for (OccupantZoneInfo zoneInfo : zoneInfos) { // Each zone
                UserHandle passenger = null;
                int zoneId = zoneInfo.zoneId;
                if (zoneId == OccupantZoneInfo.INVALID_ZONE_ID) {
                    break;
                }
                boolean isSuccess = AddNewPassengersUser(zoneId);
                if (isSuccess) {
                    ++passengersSize;
                }
            }
    ...
    public boolean AddNewPassengersUser(int zoneId) {
        ZoneUserBindingHelper helper = null;
        synchronized (mLock) {
            if (mZoneUserBindingHelper == null) {
                Slogf.e(TAG, "implementation is not delegated");
        ...
        if (getCarOccupantZoneService().getOccupantZoneIdForUserId(currentUser) == zoneId) {
            Slogf.e(TAG, "User Driver already assigned to this zone! Return!");
            return false;
        }

        List<UserHandle> userHandles = getPassengers(currentUser);

        boolean isShouldCreate = false;
        UserHandle passenger = null;
        for (UserHandle userHandle : userHandles) {
            if (getCarOccupantZoneService().getOccupantZoneIdForUserId(userHandle.getIdentifier()) == zoneId) {
                Slogf.e(TAG, "User(" + userHandle.getIdentifier() + ") already assigned to this zone(" + zoneId
                        + ")! Don't Create!");
                isShouldCreate = true;
                passenger = userHandle;
                break;
            } else if (getCarOccupantZoneService().getOccupantZoneIdForUserId(
                    userHandle.getIdentifier()) == CarOccupantZoneManager.OccupantZoneInfo.INVALID_ZONE_ID) {
                Slogf.e(TAG, "User(" + userHandle.getIdentifier()
                        + ") has not assigned to any zone yet! Assign to this Zone(" + zoneId + ")! Don't Create!");
                isShouldCreate = true;
                passenger = userHandle;
                break;
            }
        ...
        if (!isShouldCreate) {
            passenger = createPassenger("Passenger" + zoneId, currentUser);
        }
        if (passenger == null) {
            // Couldn't create user, most likely because there are too many.
            Slogf.w(TAG, "AddNewPassengersUser cannot create a passenger user");
            return false;
        }
        int passengerId = passenger.getIdentifier();
        if (!startPassenger(passengerId, zoneId)) {
            Slogf.w(TAG, "AddNewPassengersUser cannot start passenger " + passengerId);
            return false;
        }
        return true;
    }
