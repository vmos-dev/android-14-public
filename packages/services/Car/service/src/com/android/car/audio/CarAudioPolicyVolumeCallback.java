...
final class CarAudioPolicyVolumeCallback extends AudioPolicy.AudioPolicyVolumeCallback {
    ...
    private int syncCurrentVolume(int zoneId, int groupId, int currentIndex) {
        if (groupId != 0) {
            return currentIndex;
        }
        int streamType = AudioManager.STREAM_MUSIC;
        int currentVolume = currentIndex;
        ...
        int currentCarVolume = mCarVolumeInfo.getGroupVolume(zoneId, groupId);
        int minCarVolume = mCarVolumeInfo.getGroupMinVolume(zoneId, groupId);
        int maxCarVolume = mCarVolumeInfo.getGroupMaxVolume(zoneId, groupId);
        float volumeFactor =  (float)(maxCarVolume - minCarVolume) / (float)(maxStreamVolume - minStreamVolume);
        int transCarVolume = minCarVolume + (int)Math.floor((currentStreamVolume  - minStreamVolume) * volumeFactor);
        if (Math.abs(transCarVolume - currentCarVolume) > volumeFactor) {
            currentVolume = transCarVolume;
        }
        if (Slogf.isLoggable(TAG_AUDIO, VERBOSE)) {
            Slogf.e(TAG_AUDIO, "currentVolume: " + currentVolume +
                    "currentStreamVolume: " + currentStreamVolume +
                    "minStreamVolume: " + minStreamVolume +
                    "maxStreamVolume: " + maxStreamVolume +
                    "currentCarVolume: " + currentCarVolume +
                    "minCarVolume: " + minCarVolume +
                    "maxCarVolume: " + maxCarVolume +
                    "transCarVolume: " + transCarVolume);
        }
    ...
    private void evaluateVolumeAdjustmentInternal(int adjustment, int zoneId) {
        ...
        currentVolume = syncCurrentVolume(zoneId, groupId, currentVolume);
