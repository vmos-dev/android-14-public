...
public final class LauncherActivity extends FragmentActivity implements AppPickedCallback,
    ...
    private Runnable mGetApp = () -> {
        ...
        isUserUnlocked = mUserManager.isUserUnlocked(mAssignedUserId);
        if (isUserUnlocked) {
            Log.d(TAG, "getAppList mAssignedUserId = " + mAssignedUserId + " isUnLock = " + isUserUnlocked);
            pinnedAppListViewModel.getPinnedAppList(mAssignedUserId).observe(this,
                data -> mPinnedAppListAdapter.setData(data));
            appListViewModel.getAppList(mAssignedUserId).observe(this, data -> mAppListAdapter.setData(data));
        } else {
            Log.d(TAG, "mAssignedUserId: " + mAssignedUserId + " is locked!");
        }
    };
    ...
    private final UserLifecycleListener mUserLifecycleListener = (event)->{
        ...
        if (event.getEventType() == CarUserManager.USER_LIFECYCLE_EVENT_TYPE_UNLOCKED) {
            if (event.getUserHandle().getIdentifier() == mAssignedUserId && !mHandler.hasCallbacks(mGetApp)
                    && !isUserUnlocked) {
                Log.d(TAG, "mAssignedUserId : " + mAssignedUserId + " is unlocked!");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Car.createCar(/* context= */ this, /* handler= */ null, Car.CAR_WAIT_TIMEOUT_WAIT_FOREVER,
                (car, ready) -> {
                    ...
                    CarUserManager mCarUserManager = (CarUserManager) car.getCarManager(Car.CAR_USER_SERVICE);
                    mCarUserManager.addListener(getMainExecutor(), mUserLifecycleListener);
                    mAssignedUserId = occupantZoneManager.getUserForDisplayId(getDisplay().getDisplayId());
                    if (!UserManager.isHeadlessSystemUserMode() && mAssignedUserId == UserHandle.USER_NULL) {
                        Log.i(TAG, "headless system mode is false, mAssignedUserId will be set UserHandle.USER_SYSTEM(" + UserHandle.USER_SYSTEM + ")!");
                        mAssignedUserId = UserHandle.USER_SYSTEM;
                    }
        ...
        mBackgroundDrawable = new TextDrawable(this, Color.WHITE, /* defaultSize= */ 64,
                "User #" + userId,
                "AssignedUserId #" + mAssignedUserId,
                "Display #" + displayId,
                "PID " + pid,
        ...
        pinnedAppListViewModel = viewModelProvider.get(PinnedAppListViewModel.class);
        pinnedAppListViewModel.getPinnedAppList(mAssignedUserId).observe(this,
                data -> mPinnedAppListAdapter.setData(data));
    ...
    protected void onResume() {
        ...
        if (!mHandler.hasCallbacks(mGetApp)) {
            mHandler.post(mGetApp);
        }
    ...
    private void launchOtherUiForInvalidUser(Car car) {
        ...
        mAssignedUserId = occupantZoneManager.getUserForDisplayId(myDisplayId);
        if (true || mAssignedUserId == getUserId()) {
        ...
        if (mAssignedUserId == UserHandle.USER_NULL) {
            Log.w(TAG, "No assigned user for Display#" + myDisplayId);
            shouldLaunchUserPicker = true;
        } else if (userManager.isUserUnlocked(UserHandle.of(mAssignedUserId))) {
            Log.i(TAG, "Will start HOME for User:" + mAssignedUserId + ",Display#:"
                    + myDisplayId);
            shouldLaunchUserPicker = false;
        } else {
            Log.i(TAG, "Will start user picker as user is not unlocked, User:" + mAssignedUserId
                    + ",Display#:" + myDisplayId);
            shouldLaunchUserPicker = true;
        }
        if (shouldLaunchUserPicker) {
            ...
            startActivityAsUser(intent, UserHandle.of(mAssignedUserId));
        }
        finish();
    }
    ...
    void launch(Intent launchIntent) {
        ...
        try {
            ...
            startActivityAsUser(launchIntent, options.toBundle(), UserHandle.of(mAssignedUserId));
        } catch (Exception e) {
    ...
    public void onAppPicked(AppEntry appEntry) {
        ...
        Set<String> pinnedApps = sp.getStringSet(mAssignedUserId + "-" + PINNED_APPS_KEY, null);
        Log.i(TAG, "appEntry " + appEntry + " is picked, pinnedApps: " + pinnedApps);
        if (pinnedApps == null) {
        ...
        editor.putStringSet(mAssignedUserId + "-" + PINNED_APPS_KEY, pinnedApps);
        editor.apply();
        mPinnedAppListAdapter.notifyDataSetChanged();
    }
    ...
    public void dump(String prefix, FileDescriptor fd, PrintWriter writer, String[] args) {
        ...
        writer.printf("%smAssignedUserId: %s\n", prefix, mAssignedUserId);
