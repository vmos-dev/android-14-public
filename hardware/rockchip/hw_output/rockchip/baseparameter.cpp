...
int BaseParameterV1::get_csc_info(struct csc_info *csc) {
	UN_USED(csc);
	return 0;
}

int BaseParameterV1::set_csc_info(struct csc_info *csc) {
	UN_USED(csc);
	return 0;
}

int BaseParameterV1::get_dci_info(struct  dci_info *dci)    {
	UN_USED(dci);
	return 0;
}

int BaseParameterV1::set_dci_info(struct  dci_info *dci)    {
	UN_USED(dci);
	return 0;
}

int BaseParameterV1::get_acm_info(struct acm_info *acm) {
	UN_USED(acm);
	return 0;
}

int BaseParameterV1::set_acm_info(struct acm_info *acm) {
	UN_USED(acm);
	return 0;
}

int BaseParameterV1::get_pq_tuning_gamma(struct gamma_lut_data *data) {
	UN_USED(data);
	return 0;
}

int BaseParameterV1::set_pq_tuning_gamma(struct gamma_lut_data *data) {
	UN_USED(data);
	return 0;
}

int BaseParameterV1::get_pq_factory_info(struct pq_factory_info *info) {
	UN_USED(info);
	return 0;
}

int BaseParameterV1::set_pq_factory_info(struct pq_factory_info *info) {
	UN_USED(info);
	return 0;
}

int BaseParameterV1::get_pq_sharp_info(struct pq_sharp_info *info) {
	UN_USED(info);
	return 0;
}

int BaseParameterV1::set_pq_sharp_info(struct pq_sharp_info *info) {
	UN_USED(info);
	return 0;
}

int BaseParameterV1::get_aipq_info(struct aipq_info *info) {
	UN_USED(info);
	return 0;
}

int BaseParameterV1::set_aipq_info(struct aipq_info *info) {
	UN_USED(info);
	return 0;
}

int BaseParameterV1::get_version(unsigned short* major_version, unsigned short* minor_version) {
	*major_version = 1;
	*minor_version = 0;
	return 0;
}
