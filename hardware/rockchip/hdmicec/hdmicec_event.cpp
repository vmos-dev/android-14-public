			...
			if (pfd[0].revents & (POLLIN)) {
				...
				if (!ret) {
					...
					if (!validcecmessage(cec_event)) {
						...
						char receivedMsg[3 * cecframe.len + 1]; // 3 characters per byte + null terminator
						int index = 0;
						for (int i = 0; i < cecframe.len; i++) {
							index += snprintf(receivedMsg + index, sizeof(receivedMsg) - index, "%02x ", cecframe.msg[i]);
						}
						ALOGI("poll receive msg: %s\n", receivedMsg);
#endif
						if (ctx->event_callback)
							ctx->event_callback(&cec_event, ctx->cec_arg);
					} else {
