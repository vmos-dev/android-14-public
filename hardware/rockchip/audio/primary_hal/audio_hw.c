...
#ifdef SUPPORT_AUDIO_HAL_HFP_HF
#include "audio_hw_hfp.h"
#endif
#include "adsp_config/platform_adsp.h"
...
#ifdef AUDIO_EQDRC
int rk_eq_drc_init(struct audio_device *adev, struct stream_out *out)
{
    char param[100];
    unsigned long frame_size = ((out->config.channels *
                                pcm_format_to_bits(out->config.format))/8);
    sprintf(param, "/data/cfg/eq_bin/Para_%dHz_%dch.bin",
            out->config.rate, out->config.channels);
    ALOGD("step 1-%s access :%d frame_size:%d", param,access(param, 0),frame_size);
    if(access(param, 0)) {
        sprintf(param, "/vendor/etc/Para_%dHz_%dch.bin",
                out->config.rate, out->config.channels);
    }
    ALOGD("step 2-%s access :%d", param,access(param, 0));
    if (!access(param, 0)) {
        ALOGD("creating rk eq_drc para file %s period size %d",
                                param, out->config.period_size);
        adev->eq_drc_api = rk_eq_drc_create(param, out->config.period_size, frame_size);
        if (adev->eq_drc_api == NULL) {
            ALOGE("crate rk eq_drc failed!");
        }
        ALOGD("rk eq_drc created %x", adev->eq_drc_api);
    } else {
        ALOGD("param %s do not exist, eq_drc created fail", param);
    }
     return 0;
}
#endif
...
static int start_output_stream(struct stream_out *out)
{
    ...
    for (int i = 0; i < out->num_configs; ++i) {
        ...
        if (out->devices[i] == AUDIO_DEVICE_OUT_SPEAKER ||
            ...
            ALOGD("%s: card = %d, device = 0x%x, addr: %s", __FUNCTION__, card, device, out->address);
...
#ifdef AUDIO_EQDRC
    if((adev->eq_drc_api == NULL) && (is_bitstream(out) == false) &&
        (out->devices[0] == AUDIO_DEVICE_OUT_SPEAKER)) {
        rk_eq_drc_init(adev, out);
    }
    if(adev->eq_drc_api)
        adev->eq_drc_api->start_record();
#endif
    return 0;
}
...
static void lock_stream(struct audio_device *adev, struct stream_out *stream)
{
    pthread_mutex_lock(&stream->lock);
    pthread_mutex_lock(&adev->lock);
}

static void unlock_stream(struct audio_device *adev, struct stream_out *stream)
{
...
static int out_standby(struct audio_stream *stream)
{
    ...
    lock_stream(adev, out);

    do_out_standby(out);

    unlock_stream(adev, out);
...
static void platform_software_set_volume(struct audio_stream_out *stream, const struct audio_port_config *config)
{
    struct stream_out *out = (struct stream_out *)stream;
    ...
    out->amplitude_ratio = pow(10,
                                (minDb + (maxDb - minDb) *
                                (gainIndex / (float)totalSteps)) / 20);

    if (gainIndex == 0) {
        out->amplitude_ratio = 0;
    }
    ALOGD("platform_software_set_volume amplitude ratio :%f", out->amplitude_ratio);
}


// Applies gain naively, assumes AUDIO_FORMAT_PCM_16_BIT and stereo output
static void platform_software_apply_gain(struct audio_stream_out *stream, void *buffer, size_t bytes)
{
    struct stream_out *out = (struct stream_out *)stream;
    ...
    for (int i = 0; i < int16_size; i++) {
        ...
        if (multiplied > INT16_MAX) {
            int16_buffer[i] = INT16_MAX;
        } else if (multiplied < INT16_MIN) {
            int16_buffer[i] = INT16_MIN;
        } else {
            int16_buffer[i] = (int16_t)multiplied;
        }
    }
}
...
#ifdef AUDIO_EQDRC
    if (adev->eq_drc_api != NULL) {
        adev->eq_drc_api->rk_eq_drc_process(buffer, buffer,
                                        bytes, 2, pcm_format_to_bits(out->config.format));
    }
#endif
        ...
        for (i = 0; i < SND_OUT_SOUND_CARD_MAX; i++)
            if (out->pcm[i]) {
                if (i == SND_OUT_SOUND_CARD_BT) {
                    ...
                    #ifndef SUPPORT_AUDIO_ADSP
                        platform_software_apply_gain(out, (void *)buffer, bytes);
                    #endif
    ...
    if (in->pcm) {
        ...
        if (pcm_get_htimestamp(in->pcm, &avail, &timestamp) == 0) {
            ...
            *time = timestamp.tv_sec * 1000000000LL + timestamp.tv_nsec;
            ret = 0;
            ALOGD("Pos: %" PRId64 " %" PRId64, *time, *frames);
        }
    }
exit:
    pthread_mutex_unlock(&in->lock);
                                   ...
                                   const char *address)
{
    ...
    ALOGD("audio hal adev_open_output_stream devices = 0x%x, flags = %d, samplerate = %d, format = 0x%x, address = %s",
          devices, flags, config->sample_rate,config->format, address);
    ...
    if (address) {
        out->address = calloc(strlen(address) + 1, sizeof(char));
        strncpy((char *)out->address, address, strlen(address));

        hashmapPut(adev->out_bus_stream_map, (void *)out->address, out);
        ...
        ALOGD("%s Routing %s", __func__, out->address);
    }
                                     ...
                                     struct audio_stream_out *stream)
{
...

    {
        ...
        device_lock(out->dev);
        if (out->address) {
            hashmapRemove(adev->out_bus_stream_map, (void *)out->address);
            free((void *)out->address);
        }
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ALOGD("%s:bt_wb_speech_enabled = %d, sco reroute=%u",
        ...
        // HFP set sampling rate
        ret = str_parms_get_str(parms, AUDIO_PARAMETER_HFP_SET_SAMPLING_RATE, value, sizeof(value));
        if (ret >= 0) {
            rate = atoi(value);
            if (rate == AUDIO_HFP_RATE_8K || rate == AUDIO_HFP_RATE_16K)
                audio_hfp_set_sample_rate(rate);
            else
               ALOGE("Unsupported rate..");
        }

        // HFP set volume
        ret = str_parms_get_str(parms, AUDIO_PARAMETER_HFP_VOLUME, value, sizeof(value));
        if (ret >= 0) {
            if (sscanf(value, "%f", &vol) == 1)
                audio_hfp_set_volume(adev, vol);
        }
#endif
...
#ifdef SUPPORT_AUDIO_ADSP
    platform_adsp_free(adev);
#endif
    free(device);
    return 0;
}
                        ...
                        const struct audio_port_config *config)
{
    ...
    struct stream_out *out = hashmapGet(adev->out_bus_stream_map, (void *)bus_address);
...
#ifdef SUPPORT_AUDIO_ADSP
        platform_adsp_set_volume(adev, bus_address, gainIndex);
#else
        platform_software_set_volume(out, config);
#endif
        pthread_mutex_unlock(&out->lock);
        ALOGD("%s: set audio index: %d on %s", __func__, gainIndex, bus_address);
    } else {
                     ...
                     hw_device_t** device)
{
    ...
    adev->out_bus_stream_map = hashmapCreate(10, str_hash_fn, str_eq);
