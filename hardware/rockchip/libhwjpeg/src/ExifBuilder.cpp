...
static const unsigned char gExifHeader[] = { 0x45, 0x78, 0x69, 0x66, 0x00, 0x00 };
...
static bool exif_save_data_content(ExifData *edata, ExifContent *ifd,
    ...
    *len = ts;
    ...
    if (i == EXIF_IFD_0 && (edata->ifd[EXIF_IFD_1].entry_count
        ...
        exif_save_data_content(edata, &edata->ifd[EXIF_IFD_1], buf, len, *len - 6);
    } else {
...
bool exif_general_build(ExifData *edata, unsigned char **buf, int *len) {
    ...
    *buf = (unsigned char*)malloc(*len);
    ...
    ALOGV("Saving IFDs...");
    if (exif_save_data_content(
            edata, &edata->ifd[EXIF_IFD_0], buf, len, (*len - 6))) {
