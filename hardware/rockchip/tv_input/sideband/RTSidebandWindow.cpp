...
int RTSidebandWindow::buffDataTransfer2(buffer_handle_t srcHandle, buffer_handle_t dstHandle) {
    if (srcHandle && dstHandle) {
        ...
        DEBUG_PRINT(mDebugLevel, "%s src %dx%d, datasize=%d", __func__, width, height, srcDatasize);
        ...
        DEBUG_PRINT(mDebugLevel, "%s dst %dx%d, datasize=%d", __func__, width, height, dstDatasize);
...
#define ENABLE_USE_SIMD_NEON 1  // enable ARM NEON

#if ENABLE_USE_SIMD_NEON
#ifdef _WIN32
...
#define FAST_MIN_MAX_NEON(a, b) \
        {                           \
            uint8x16_t t = a;       \
            a = vminq_u8(a, b);     \
            b = vmaxq_u8(b, t);     \
        }
#endif // ENABLE_USE_SIMD_NEON

#define FAST_MIN_MAX(a, b)                \
        {                                     \
            int t = (a - b <= 0) ? 0 : a - b; \
            a -= t;                           \
            b += t;                           \
...
void MedianBlur3x3_RGB(RK_U8* pSrc, int nImgHgt, int nImgWid, int nSrcStride, RK_U8* pDst, int nDstStride)
{
    const int nStaRow = 0;
    const int nEndRow = nImgHgt;

    const int nElements = 3;    // rgb format
    const int nWinSize = 3;     // 3x3
    const int nRadius = nWinSize / 2;
    const int nStep = nRadius * nElements;
    ...
    {
        RK_U8* pDstRow  = pDst + y * nDstStride;
        RK_U8* pSrcRow0 = pSrc + MAX(y - 1, 0) * nSrcStride;
        RK_U8* pSrcRow1 = pSrc + y * nSrcStride;
        RK_U8* pSrcRow2 = pSrc + MIN(y + 1, nImgHgt - 1) * nSrcStride;

        xLimit = nRadius * nElements;
        x1 = 0;
        while (1)
        {
            // left, right
            for (; x1 < xLimit; x1 += nElements)
            {
                x0 = MAX(x1 - nElements, 0);
                x2 = MIN(x1 + nElements, xLimit - nElements);
                // R
                p0 = pSrcRow0[x0], p1 = pSrcRow0[x1], p2 = pSrcRow0[x2];
                p3 = pSrcRow1[x0], p4 = pSrcRow1[x1], p5 = pSrcRow1[x2];
                p6 = pSrcRow2[x0], p7 = pSrcRow2[x1], p8 = pSrcRow2[x2];
                FAST_MIN_MAX(p1, p2); FAST_MIN_MAX(p4, p5); FAST_MIN_MAX(p7, p8); FAST_MIN_MAX(p0, p1);
                FAST_MIN_MAX(p3, p4); FAST_MIN_MAX(p6, p7); FAST_MIN_MAX(p1, p2); FAST_MIN_MAX(p4, p5);
                FAST_MIN_MAX(p7, p8); FAST_MIN_MAX(p0, p3); FAST_MIN_MAX(p5, p8); FAST_MIN_MAX(p4, p7);
                FAST_MIN_MAX(p3, p6); FAST_MIN_MAX(p1, p4); FAST_MIN_MAX(p2, p5); FAST_MIN_MAX(p4, p7);
                FAST_MIN_MAX(p4, p2); FAST_MIN_MAX(p6, p4); FAST_MIN_MAX(p4, p2);
                pDstRow[x1] = p4;
                // G
                p0 = pSrcRow0[x0 + 1], p1 = pSrcRow0[x1 + 1], p2 = pSrcRow0[x2 + 1];
                p3 = pSrcRow1[x0 + 1], p4 = pSrcRow1[x1 + 1], p5 = pSrcRow1[x2 + 1];
                p6 = pSrcRow2[x0 + 1], p7 = pSrcRow2[x1 + 1], p8 = pSrcRow2[x2 + 1];
                FAST_MIN_MAX(p1, p2); FAST_MIN_MAX(p4, p5); FAST_MIN_MAX(p7, p8); FAST_MIN_MAX(p0, p1);
                FAST_MIN_MAX(p3, p4); FAST_MIN_MAX(p6, p7); FAST_MIN_MAX(p1, p2); FAST_MIN_MAX(p4, p5);
                FAST_MIN_MAX(p7, p8); FAST_MIN_MAX(p0, p3); FAST_MIN_MAX(p5, p8); FAST_MIN_MAX(p4, p7);
                FAST_MIN_MAX(p3, p6); FAST_MIN_MAX(p1, p4); FAST_MIN_MAX(p2, p5); FAST_MIN_MAX(p4, p7);
                ...
                pDstRow[x1 + 1] = p4;
                // B
                p0 = pSrcRow0[x0 + 2], p1 = pSrcRow0[x1 + 2], p2 = pSrcRow0[x2 + 2];
                p3 = pSrcRow1[x0 + 2], p4 = pSrcRow1[x1 + 2], p5 = pSrcRow1[x2 + 2];
                p6 = pSrcRow2[x0 + 2], p7 = pSrcRow2[x1 + 2], p8 = pSrcRow2[x2 + 2];
                FAST_MIN_MAX(p1, p2); FAST_MIN_MAX(p4, p5); FAST_MIN_MAX(p7, p8); FAST_MIN_MAX(p0, p1);
                FAST_MIN_MAX(p3, p4); FAST_MIN_MAX(p6, p7); FAST_MIN_MAX(p1, p2); FAST_MIN_MAX(p4, p5);
                FAST_MIN_MAX(p7, p8); FAST_MIN_MAX(p0, p3); FAST_MIN_MAX(p5, p8); FAST_MIN_MAX(p4, p7);
                FAST_MIN_MAX(p3, p6); FAST_MIN_MAX(p1, p4); FAST_MIN_MAX(p2, p5); FAST_MIN_MAX(p4, p7);
            ...
            if (xLimit == nImgWid * nElements) {
                break;
            }

            // middle
            xLimit = nImgWid * nElements;
    #if ENABLE_USE_SIMD_NEON
            for (; x1 <= xLimit - 16 * nElements; x1 += 16 * nElements)
            {
                uint8x16x3_t vsrc0 = vld3q_u8(pSrcRow0 + x1 - nStep), vsrc1 = vld3q_u8(pSrcRow0 + x1), vsrc2 = vld3q_u8(pSrcRow0 + x1 + nStep);
                uint8x16x3_t vsrc3 = vld3q_u8(pSrcRow1 + x1 - nStep), vsrc4 = vld3q_u8(pSrcRow1 + x1), vsrc5 = vld3q_u8(pSrcRow1 + x1 + nStep);
                uint8x16x3_t vsrc6 = vld3q_u8(pSrcRow2 + x1 - nStep), vsrc7 = vld3q_u8(pSrcRow2 + x1), vsrc8 = vld3q_u8(pSrcRow2 + x1 + nStep);
                uint8x16x3_t vdst;
                uint8x16_t p0, p1, p2, p3, p4, p5, p6, p7, p8;
                // R
                p0 = vsrc0.val[0], p1 = vsrc1.val[0], p2 = vsrc2.val[0];
                p3 = vsrc3.val[0], p4 = vsrc4.val[0], p5 = vsrc5.val[0];
                p6 = vsrc6.val[0], p7 = vsrc7.val[0], p8 = vsrc8.val[0];
                FAST_MIN_MAX_NEON(p1, p2); FAST_MIN_MAX_NEON(p4, p5); FAST_MIN_MAX_NEON(p7, p8); FAST_MIN_MAX_NEON(p0, p1);
                FAST_MIN_MAX_NEON(p3, p4); FAST_MIN_MAX_NEON(p6, p7); FAST_MIN_MAX_NEON(p1, p2); FAST_MIN_MAX_NEON(p4, p5);
                FAST_MIN_MAX_NEON(p7, p8); FAST_MIN_MAX_NEON(p0, p3); FAST_MIN_MAX_NEON(p5, p8); FAST_MIN_MAX_NEON(p4, p7);
                FAST_MIN_MAX_NEON(p3, p6); FAST_MIN_MAX_NEON(p1, p4); FAST_MIN_MAX_NEON(p2, p5); FAST_MIN_MAX_NEON(p4, p7);
                FAST_MIN_MAX_NEON(p4, p2); FAST_MIN_MAX_NEON(p6, p4); FAST_MIN_MAX_NEON(p4, p2);
                vdst.val[0] = p4;
                // G
                p0 = vsrc0.val[1], p1 = vsrc1.val[1], p2 = vsrc2.val[1];
                p3 = vsrc3.val[1], p4 = vsrc4.val[1], p5 = vsrc5.val[1];
                p6 = vsrc6.val[1], p7 = vsrc7.val[1], p8 = vsrc8.val[1];
                FAST_MIN_MAX_NEON(p1, p2); FAST_MIN_MAX_NEON(p4, p5); FAST_MIN_MAX_NEON(p7, p8); FAST_MIN_MAX_NEON(p0, p1);
                FAST_MIN_MAX_NEON(p3, p4); FAST_MIN_MAX_NEON(p6, p7); FAST_MIN_MAX_NEON(p1, p2); FAST_MIN_MAX_NEON(p4, p5);
                FAST_MIN_MAX_NEON(p7, p8); FAST_MIN_MAX_NEON(p0, p3); FAST_MIN_MAX_NEON(p5, p8); FAST_MIN_MAX_NEON(p4, p7);
                FAST_MIN_MAX_NEON(p3, p6); FAST_MIN_MAX_NEON(p1, p4); FAST_MIN_MAX_NEON(p2, p5); FAST_MIN_MAX_NEON(p4, p7);
                ...
                vdst.val[1] = p4;
                // B
                p0 = vsrc0.val[2], p1 = vsrc1.val[2], p2 = vsrc2.val[2];
                p3 = vsrc3.val[2], p4 = vsrc4.val[2], p5 = vsrc5.val[2];
                p6 = vsrc6.val[2], p7 = vsrc7.val[2], p8 = vsrc8.val[2];
                FAST_MIN_MAX_NEON(p1, p2); FAST_MIN_MAX_NEON(p4, p5); FAST_MIN_MAX_NEON(p7, p8); FAST_MIN_MAX_NEON(p0, p1);
                FAST_MIN_MAX_NEON(p3, p4); FAST_MIN_MAX_NEON(p6, p7); FAST_MIN_MAX_NEON(p1, p2); FAST_MIN_MAX_NEON(p4, p5);
                FAST_MIN_MAX_NEON(p7, p8); FAST_MIN_MAX_NEON(p0, p3); FAST_MIN_MAX_NEON(p5, p8); FAST_MIN_MAX_NEON(p4, p7);
                FAST_MIN_MAX_NEON(p3, p6); FAST_MIN_MAX_NEON(p1, p4); FAST_MIN_MAX_NEON(p2, p5); FAST_MIN_MAX_NEON(p4, p7);
...
int RTSidebandWindow::medianblur(buffer_handle_t srcHandle, buffer_handle_t dstHandle) {
    if (srcHandle && dstHandle) {
#if 0
        readDataFromFile("/data/test.yuv", srcHandle);
#endif
        RK_U8* tmpSrcPtr = NULL;
        RK_U8* tmpDstPtr = NULL;
        int srcDatasize = -1;
        int lockMode = GRALLOC_USAGE_SW_READ_MASK | GRALLOC_USAGE_SW_WRITE_MASK | GRALLOC_USAGE_HW_CAMERA_MASK;
        ...
        ALOGD("data tmpSrcPtr ptr = %p, srcDatasize=%d", tmpSrcPtr, srcDatasize);
        int dstWidth = mBuffMgr->GetWidth(dstHandle);
        int dstHeight =  mBuffMgr->GetHeight(dstHandle);
        srcDatasize = 0;
        for (int i = 0; i < mBuffMgr->GetNumPlanes(dstHandle); i++) {
            srcDatasize += mBuffMgr->GetPlaneSize(dstHandle, i);
        }

        ALOGD("data tmpDstPtr ptr = %p, width=%d, height=%d, dataSize=%d",
            tmpDstPtr, dstWidth, dstHeight, srcDatasize);

        mBuffMgr->LockLocked(dstHandle, lockMode, 0, 0, dstWidth, dstHeight, (void**)&tmpDstPtr);

        int debugValue = 0;
#if 1
        char propValue[PROPERTY_VALUE_MAX] = {0};
        property_get(TV_INPUT_PQ_MEDIAN_BLUE, propValue, "0");
        debugValue = (int)atoi(propValue);
#endif
        if (debugValue == 1) {
            int width_stride = _ALIGN(srcWidth * 3, 64);
            MedianBlur3x3_RGB(tmpSrcPtr, srcHeight, srcWidth, width_stride, tmpDstPtr, width_stride);
        } else {
            std::memcpy(tmpDstPtr, tmpSrcPtr, srcDatasize);
        }
...
void RTSidebandWindow::readDataFromFile(const char *file_path, buffer_handle_t dstHandle) {
    ...
    if (stat(file_path, &statbuff) < 0) {
        ALOGE("stat file %s error", file_path);
        fclose(fp);
        return;
    }
