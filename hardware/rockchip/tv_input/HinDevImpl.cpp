...
int HinDevImpl::start()
{
    ...
    mState = START;
    memset(prop_value, '\0', sizeof(prop_value));
    property_get(TV_INPUT_PQ_MEDIAN_BLUE, prop_value, "0");
    mMedianBlurMode = (int)atoi(prop_value);
        ...
        if (mPqBufferHandle.empty()) {
            ...
            for (int i=0; i<mPqBufferHandle.size(); i++) {
                if (mFrameType & TYPE_SIDEBAND_WINDOW) {
                    ...
                    if ((tempPqMode & PQ_MEDIAN_BLUR) == PQ_MEDIAN_BLUR) {
                        ALOGW("pq allocateBuffer for MEDIAN_BLUR %dx%d %d", mDstFrameWidth, mDstFrameHeight, tempPqMode);
...
int HinDevImpl::getOutRange(char* value) {
    ...
    //vop don't support 709 full output, so use the auto
    return ret;
}
...
//start encode threads
            ...
            if (mMedianBlurMode > 0 && V4L2_PIX_FMT_BGR24 == mPixelFormat
                    && mSrcFrameWidth == 720 && mSrcFrameHeight == 480
                    && (mPqMode & PQ_MEDIAN_BLUR) != PQ_MEDIAN_BLUR) {
                if (mSkipFrame <= 0) {
                    mSkipFrame = 1;
                }
                        ...
                        if (mPqMode != PQ_OFF && !mPqPrepareList.empty()) {
                            for (int i = 0; i < mPqPrepareList.size(); i++) {
                                ...
                                if (!mPqBufferHandle[pqBufIndex].isFilled) {
                                    ...
                                    if ((mPqMode & PQ_MEDIAN_BLUR) == PQ_MEDIAN_BLUR) {
                                        mPqBufferHandle[pqBufIndex].srcHandle = mHinNodeInfo->vt_buffers[currDqbufHandleIndex]->handle;
                                    }
...
bool HinDevImpl::needShowPqFrame(int pqMode) {
    if ((pqMode & PQ_LF_RANGE) == PQ_LF_RANGE
            ...
            || (pqMode & PQ_MEDIAN_BLUR) == PQ_MEDIAN_BLUR) {
        ...
        if (mFrameType & TYPE_SIDEBAND_VTUNNEL) {
            if (mPqMode == PQ_CACL_LUMA) {
                ...
                if (mPqBufferHandle[pqBufIndex].isFilled) {
                    ...
                    } else if ((mPqMode & PQ_MEDIAN_BLUR) == PQ_MEDIAN_BLUR) {
                        mSidebandWindow->medianblur(mPqBufferHandle[pqBufIndex].srcHandle,
                            mPqBufferHandle[pqBufIndex].out_vt_buffer->handle);
                    }
                    qBuf(mPqBufferHandle[pqBufIndex].src_vt_fd, true);
