...
V4L2EventCallBack hinDevEventCallback(int event_type) {
	...
	case V4L2_EVENT_CTRL:
        ...
        if (!isHdmiIn) {
            ...
            //notify audio_present=0 to manager when isHdmiIn false
            if (s_TvInputPriv->mDev) {
                tv_input_event_ext_t event_audio;
                event_audio.base_event.type = TV_INPUT_EVENT_PRIV_CMD_TO_APP;
                event_audio.priv_app_cmd.action = "audio_present=0";
                s_TvInputPriv->callback->notify_ext(nullptr, &event_audio, nullptr);
            }
        }
             break;
        ...
        case RK_HDMIRX_V4L2_EVENT_AUDIOINFO:
            if (s_TvInputPriv->mDev && s_TvInputPriv->mDev->started()) {
                ...
                event.base_event.type = TV_INPUT_EVENT_PRIV_CMD_TO_APP;
                char audio_present[30];
                sprintf(audio_present, "audio_present=%d", present);
                event.priv_app_cmd.action = audio_present;
             }
             break;
    }
