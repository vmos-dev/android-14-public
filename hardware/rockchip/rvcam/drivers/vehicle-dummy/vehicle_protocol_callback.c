...
bool decode_prop_callback(pb_istream_t *stream, const pb_field_t *field,
			  void **arg)
{
...
bool decode_config_callback(pb_istream_t *stream, const pb_field_t *field,
			    void **arg)
{
	...
	propconfigure.config_string.funcs.decode =
		&decode_config_string_callback;
				  ...
				  void **arg)
{
	...
	if (!pb_decode_fixed32(stream, &value)) {
		pr_err("float_values_callback failed\n");
		return false;
	}
...
bool decode_value_callback(pb_istream_t *stream, const pb_field_t *field,
			   void **arg)
{
	struct vehicle_property_set *data =
		(struct vehicle_property_set *)(*arg);
				  ...
				  void **arg)
{
	...
	if (!pb_decode(stream, emulator_VehicleAreaConfig_fields,
		       &vehicle_area))
		return false;
	return true;
}
...
bool decode_config_string_callback(pb_istream_t *stream,
...
bool encode_prop_callback(pb_ostream_t *stream, const pb_field_t *field,
			  void *const *arg)
{
	emulator_VehiclePropGet propget;
				  ...
				  void *const *arg)
{
	...
	if (!pb_encode(stream, emulator_VehicleAreaConfig_fields,
		       &vehicle_area))
		return false;
	return true;
}
...
bool encode_config_string_callback(pb_ostream_t *stream,
...
bool encode_config_callback(pb_ostream_t *stream, const pb_field_t *field,
			    void *const *arg)
{
	emulator_VehiclePropConfig propconfigure;
	...
	propconfigure.config_string.funcs.encode =
		&encode_config_string_callback;
...
bool encode_power_state_value_callback(pb_ostream_t *stream,
				 ...
				 void *const *arg)
{
	...
	propvalue.status = 0;
#endif
	propvalue.int32_values.funcs.encode =
		&encode_power_state_value_callback;
	propvalue.int32_values.arg = &data->state;
	propvalue.value_type = VEHICLEPROPERTYTYPE_INT32;

	pb_encode_tag_for_field(stream, field);
	if (!pb_encode_submessage(stream, emulator_VehiclePropValue_fields,
				  &propvalue)) {
		pr_err("%s encode submessage failed\n", __func__);
		return false;
	}
	return true;
}
			   ...
			   void *const *arg)
{
	...
	if (HVAC_TEMPERATURE_SET == data->prop) {
		propvalue.float_values.funcs.encode =
			&encode_fix32_values_callback;
		propvalue.float_values.arg = &data->value;
		propvalue.value_type = VEHICLEPROPERTYTYPE_FLOAT;
	} else if (HVAC_FAN_SPEED == data->prop ||
		   HVAC_FAN_DIRECTION == data->prop ||
		   HVAC_AUTO_ON == data->prop || HVAC_AC_ON == data->prop ||
		   HVAC_RECIRC_ON == data->prop ||
		   HVAC_POWER_ON == data->prop ||
		   HVAC_DEFROSTER == data->prop ||
		   TURN_SIGNAL_STATE == data->prop ||
		   GEAR_SELECTION == data->prop ||
		   HVAC_SEAT_TEMPERATURE == data->prop) {
		propvalue.int32_values.funcs.encode =
			&encode_int32_values_callback;
		propvalue.int32_values.arg = &data->value;
		propvalue.value_type = VEHICLEPROPERTYTYPE_INT32;
	}
