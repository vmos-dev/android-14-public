	...
	if ((byte & 0x80) == 0) {
		...
		do {
			...
			if (bitpos >= 32) {
				...
				uint8_t sign_extension = (bitpos < 63) ? 0xFF :
									       0x01;
	...
	if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
		...
		if (init_data) {
			if (PB_LTYPE(iter->pos->type) == PB_LTYPE_SUBMESSAGE) {
				...
				memcpy(iter->pData, iter->pos->ptr,
				       iter->pos->data_size);
			} else {
				/* Initialize to zeros */
				memset(iter->pData, 0, iter->pos->data_size);
...
{
	pb_size_t old_tag =
		*(pb_size_t *)iter->pSize; /* Previous which_ value */
...
static void pb_release_single_field(const pb_field_iter_t *iter)
{
	...
	if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
		...
		if (pItem) {
			while (count--) {
				pb_release((const pb_field_t *)iter->pos->ptr,
					   pItem);
...
bool pb_decode_fixed32(pb_istream_t *stream, void *dest)
{
	...
	*(uint32_t *)dest =
		((uint32_t)bytes[0] << 0) | ((uint32_t)bytes[1] << 8) |
						  ...
						  void *dest)
{
	...
	if (size == 0) {
		/* As a special case, treat empty bytes string as
		 * all zeros for fixed_length_bytes.
		 */
