...
struct RkCamera : public HwCamera {
    struct Parameters {
        std::string name;
    ...
    explicit RkCamera(sp<CameraModule> module,std::string cameraId/*const Parameters& params*/);
    ...
    std::string getCameraId() const override;
    sp<CameraModule> getModule() const override;
    camera3_device_t* getDevice() const override;
    void setDevice(camera3_device_t* device) override;
    std::vector<StreamBuffer> getOutputStreamBuffer(HwCaptureRequest req,int streamId) override;
private:
    struct StreamInfo {
        Rect<uint16_t> size;
