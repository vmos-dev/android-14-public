...
const char* HdmiConfig::kDefaultCfgPath = "/vendor/etc/hdmi_config.xml";

HdmiConfig HdmiConfig::loadFromCfg(const char* cfgPath) {
    ...
    if (err != XML_SUCCESS) {
        ALOGE("%s: Unable to load hdmi config file '%s'. Error: %s", __FUNCTION__,
              cfgPath, XMLDocument::ErrorIDToName(err));
        return ret;
    } else {
        ALOGI("%s: load hdmi config succeeded!", __FUNCTION__);
    }

    XMLElement* extCam = configXml.FirstChildElement("ExternalCamera");
    if (extCam == nullptr) {
        ALOGI("%s: no hdmi config specified", __FUNCTION__);
        return ret;
    }

    XMLElement* providerCfg = extCam->FirstChildElement("Provider");
    if (providerCfg == nullptr) {
        ALOGI("%s: no hdmi provider config specified", __FUNCTION__);
        return ret;
    }
    ...
    while (id != nullptr) {
        ...
        if (text != nullptr) {
            ...
            ALOGI("%s: device %s will be ignored by hdmi provider", __FUNCTION__, text);
        }
    ...
    if (deviceCfg == nullptr) {
        ALOGI("%s: no hdmi device config specified", __FUNCTION__);
        return ret;
    }
    ...
    ALOGI("%s: hdmi cfg loaded: maxJpgBufSize %d,"
...
bool HdmiConfig::updateFpsList(tinyxml2::XMLElement* fpsList,
