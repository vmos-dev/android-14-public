...
using ::android::hardware::camera::provider::implementation::HdmiProvider;
...
int main() {
    ALOGI("HdmiProvider: hdmi service is starting.");

    ABinderProcess_setThreadPoolMaxThreadCount(HWBINDER_THREAD_COUNT);

    std::shared_ptr<HdmiProvider> defaultProvider =
            ndk::SharedRefBase::make<HdmiProvider>();
    const std::string serviceName = std::string(HdmiProvider::descriptor) + "/hdmi/0";

#ifdef LAZY_SERVICE
    binder_exception_t ret = AServiceManager_registerLazyService(defaultProvider->asBinder().get(),
                                                                 serviceName.c_str());
    LOG_ALWAYS_FATAL_IF(ret != EX_NONE,
                        "Error while registering lazy hdmi provider service: %d", ret);
#else
    binder_exception_t ret =
            AServiceManager_addService(defaultProvider->asBinder().get(), serviceName.c_str());
    LOG_ALWAYS_FATAL_IF(ret != EX_NONE, "Error while registering hdmi provider service: %d",
                        ret);
#endif
