...
static int hal_uevent_sock_fd = -1;
...
struct platform_app_map
{
    char modalias[30];
    char app[30];
};

static platform_app_map map_table[] = {
    {"platform:regulatory", "logwrapper /vendor/bin/crda"},
};
...
wifi_error wifi_initialize(wifi_handle *handle)
{
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, info->uevent_cleanup_socks) == -1) {
        ALOGE("Could not create cleanup sockets");
        free(info);
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    hal_uevent_sock_fd = uevent_open_socket(5*2048, true);
    if (hal_uevent_sock_fd < 0) {
        ALOGE("fail to init uevent_socket");
         close(hal_uevent_sock_fd);
        return WIFI_ERROR_UNKNOWN;
    }
...
static void internal_cleaned_up_handler(wifi_handle handle)
{
    ...
    if (info->cmd_sock != 0) {
        ...
        close(info->uevent_cleanup_socks[0]);
        close(info->uevent_cleanup_socks[1]);
...
const char *parse_event(const char *msg, struct listener_event *event){
    ...
    while (*msg) {
        if (!strncmp(msg, "ACTION=", 7)) {
            ...
            msg += 6;
            event->major = atoi(msg);
        } else if (!strncmp(msg, "MINOR=", 6)) {
            msg += 6;
            event->minor = atoi(msg);
        } else if (!strncmp(msg, "PARTN=", 6)) {
            msg += 6;
            event->partition_num = atoi(msg);
        } else if (!strncmp(msg, "PARTNAME=", 9)) {
            msg += 9;
            event->partition_name = msg;
        } else if (!strncmp(msg, "DEVNAME=", 8)) {
            msg += 8;
            event->device_name = msg;
    ...
    return (event->modalias) ;

}

void eventenv_set(char *msg){
    ...
    while (*msg) {
        if (!strncmp(msg, "ACTION=", 7)) {
            ...
            if(env_num != 0){
                event_env_name = msg;
                char *eq_p = strchr(msg, '=');
                event_env_data = eq_p +1;
                *eq_p = '\0';
                if(setenv(event_env_name,event_env_data,1)!=0){
                    ALOGD("setenv error\n");
                }
                *eq_p = '=';
            }
...
void *uevent_loop(void *arg)
{
    hal_info *info = (hal_info *)arg;
    pollfd pfd[2];
    memset(&pfd[0], 0, sizeof(pollfd));

    pfd[0].fd = hal_uevent_sock_fd;
    pfd[0].events = POLLIN;
    pfd[1].fd = info->uevent_cleanup_socks[1];
    pfd[1].events = POLLIN;
    ...
    do {
        ...
        if (result < 0) {
            ...
            uid_t uid = -1;
            uid_t *UID =&uid;
            struct listener_event event;
            int n = uevent_kernel_recv(hal_uevent_sock_fd, buf, sizeof(buf), true, UID);
            
            if(n > 0 && n < UEVENT_MSG_LEN && errno == 0){
                buf[n] = '\0';
                buf[n + 1] = '\0';

                const char *event_modalias=parse_event(buf, &event);
                for(int i = 0; i < (sizeof(map_table)/sizeof(map_table[0])); i++){
                    
                    if(!strncmp(event_modalias, map_table[i].modalias, strlen(map_table[i].modalias))){
                        eventenv_set(buf);
                        if (system(map_table[i].app) != 0)
                            ALOGE("system order fail\n");
                        eventenv_unset(buf);
                    }
                }
            }
            ...
            ALOGE("Unknown event - %0x, %0x, %0x", pfd[0].revents, pfd[1].revents);
        }
    } while (!info->uevent_loop_stop);
...
void wifi_event_loop(wifi_handle handle)
{
    ...
    /* create a thread to receive uevent message */
    /* if receive uevent in current thread, uevent receive will be blocked when set country */
    int ret = pthread_create(&uevent_thread, NULL, uevent_loop, info);
    if(ret != 0) {
        ALOGE("Create Uevent Loop thread failed");
    }
    ...
    /* notify uevent thread to exit */
    if (TEMP_FAILURE_RETRY(write(info->uevent_cleanup_socks[0], "Exit", 4)) < 1) {
        // As a fallback set the cleanup flag to TRUE
        ALOGE("could not write to the uevent cleanup socket");
    } else {
        // Listen to the response
        // Hopefully we dont get errors or get hung up
        // Not much can be done in that case, but assume that
        ...
        // As a fallback set the cleanup flag to TRUE
        memset(buf, 0, sizeof(buf));
        ssize_t result = TEMP_FAILURE_RETRY(read(info->uevent_cleanup_socks[0], buf, sizeof(buf)));
        ALOGE("%s: Read after POLL returned %zd, error no = %d (%s)", __FUNCTION__,
               result, errno, strerror(errno));
        if (strncmp(buf, "Done", 4) == 0) {
            ALOGE("UEvent processing terminated");
        } else {
            ALOGD("Rx'ed %s", buf);
        }
    }

    /* wait until uevent loop exit */
    info->uevent_loop_stop = true;
...
public:
    ...
    virtual int handleEvent(WifiEvent& event) {
        ...
        uint8_t type = event.get_u8(NL80211_ATTR_REG_TYPE);
        if(type == NL80211_REGDOM_TYPE_COUNTRY) {
            char *alpha2 = (char *)event.get_data(NL80211_ATTR_REG_ALPHA2);
            if(alpha2[0] != mCountryCode[0]
                || alpha2[1] != mCountryCode[1]) {
                    ALOGW("Event country info mismatch %c%c, expected %c%c", 
                        alpha2[0], alpha2[1], mCountryCode[0], mCountryCode[1]);
            }
        } else {
            ALOGW("Reg Type is not a country");
        }
...
{
    wifi_error error = WIFI_SUCCESS;
    SetCountryCodeCommand command(handle, country_code);
    error = (wifi_error)command.requestEvent(NL80211_CMD_REG_CHANGE);
