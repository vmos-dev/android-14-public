...
#if (UPIO_DBG == TRUE)
#define UPIODBG(param, ...) {ALOGD("BES_ "param, ## __VA_ARGS__);}
#else
...
static int driver_send_cmd_and_get_reply(const char *wiphy, const char *cmd, char *reply, uint32_t buf_len)
{
        ...
        write_len += strlen(wiphy);
        write_len += 1; // the length of whitespace
        write_len += 4; // the length of "cmd:"
        write_len += strlen(cmd);
        write_len += 1; // the length of '\0'
        cmd_str = (char *)malloc(write_len);
        if(!cmd_str)
                return -3;

        write_len = snprintf(cmd_str, write_len, "ifname:%s cmd:%s", wiphy, cmd);
...
static int bt_send_cmd(const char* cmd){
    int ret = 0;
    const char* dirver_wiphy = "NONE";
    char* reply;
    int reply_len = 4096;
    char *bt_opened_flag;
    reply = (char *)malloc(reply_len);
    if (reply == NULL){
        ALOGE("failed to malloc reply");
        return -1;
    }

    if ((ret = driver_send_cmd_and_get_reply(dirver_wiphy, cmd, reply, reply_len)) < 0) {
        ALOGE("failed to issue private command: '%s', ret: %d\n", cmd, ret);
        ret = -1;
    }else{
        bt_opened_flag = strstr(reply, "bt_opened:");

        if((!strncmp("1", bt_opened_flag+10, 1)) && (!strncmp("BT_ON", cmd, 5))){
            ret = 0;
        }else if((!strncmp("0", bt_opened_flag+10, 1)) && (!strncmp("BT_OFF", cmd, 6))){
            ret = 0;
        }else{
            ret = -1;
        }
...
int upio_set_bluetooth_power(int on)
{
    ...
    const char* cmd_on = "BT_ON";
    const char* cmd_off = "BT_OFF";

   /* send cmd: BT_ON */
    if(on == UPIO_BT_POWER_ON){
        if(bt_send_cmd(cmd_on) == 0)
            ALOGI("send cmd '%s' success\n",cmd_on);
        else
            ALOGE("send cmd '%s' failed\n",cmd_on);
    }

    /* send cmd: BT_OFF */
    if(on == UPIO_BT_POWER_OFF){
        if(bt_send_cmd(cmd_off) == 0)
            ALOGI("send cmd '%s' success\n",cmd_off);
        else
            ALOGE("send cmd '%s' failed\n",cmd_off);
    }
