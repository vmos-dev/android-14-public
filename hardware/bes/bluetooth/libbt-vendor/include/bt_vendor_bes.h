...
#ifndef BT_VENDOR_BES_H
#define BT_VENDOR_BES_H
...
#define VENDOR_LIB_CONF_FILE "/vendor/etc/bluetooth/besbt.conf"
#endif

/* Device port name where Bluetooth controller attached */
#ifndef BLUETOOTH_UART_DEVICE_PORT
#define BLUETOOTH_UART_DEVICE_PORT      "/dev/ttyS2"    /* maguro */
#endif
...
#define BES_GET_BOUNDARY_FLAG(handle) (((handle) >> 12) & 0x0003)

#define ASSERT(condition) \
  if (!(condition)) { \
    ALOGE("%s:%d assertion '" #condition "' failed", __FILE__, __LINE__); \
    abort(); \
  }
